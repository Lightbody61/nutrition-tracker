...
for(const label of ['Proceed to Tracker','Reports','AI Assistance','Users Guide','Community Forum','Back to Account']) assert.ok(mainMenuMarkup.includes(`>${label}</button>`),`missing Main Menu button: ${label}`);
assert.ok(!mainMenuMarkup.includes('>Contact Admin</button>'));
for(const label of ['Food','Exercise','Utilities','Profile']) assert.ok(!mainMenuMarkup.includes(`>${label}</button>`),`tracker button leaked into Main Menu: ${label}`);
...
assert.strictEqual((homeMarkup.match(/<button\b/g)||[]).length,5);
for(const label of ['Food','Exercise','Profile','Utilities','Main Menu']) assert.ok(homeMarkup.includes(`>${label}</button>`),`missing Tracker Menu button: ${label}`);
assert.ok(!homeMarkup.includes('>Users Guide</button>'),'Users Guide should not remain in Tracker Menu');
...
assert.ok(html.includes("usersGuideScreen:'mainMenuScreen'"));
...
assert.ok(html.includes('@media(max-width:599px){.homeMenu{grid-template-columns:1fr;gap:5px;padding:4px 0}'));
assert.ok(html.includes('.homeMenu button{min-height:48px;font-size:15px;line-height:1.1;padding:7px}'));
