...
async function runCopyButtonRegressionTests(){
 ...
 assert.ok(h.elements.aiGeneratedMealPlanInstructions.value.includes('Date range: 2026-08-20 through 2026-08-20.'));assert.strictEqual(h.elements.aiMealPlanCopyStatus.textContent,'Instructions copied successfully.');assert.ok(!h.elements.aiMealPlanCopyStatus.classList.contains('error'));assert.strictEqual(h.elements.aiPasteMealPlanBtn.disabled,false);assert.strictEqual(h.body.children.length,0,'temporary clipboard textarea should be removed after menu fallback');
