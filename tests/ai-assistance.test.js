...
async function runCopyButtonRegressionTests(){
 ...
 function harness({clipboardMode='success',secure=true,execMode=true}={}){
  ...
  const navigator={clipboard:{writeText:async text=>{if(clipboardMode==='reject')throw new Error('blocked');clipboardText=String(text);writes.push(clipboardText);return true;},readText:async()=>{reads.push(clipboardText);return clipboardText;}}};
 ...
 assert.strictEqual(h.writes.length,3);assert.strictEqual(h.elements.aiMealPlanCopyStatus.textContent,'Choose a valid start date.');assert.ok(h.elements.aiMealPlanStart.classList.contains('aiInvalid'));assert.strictEqual(h.elements.aiMealPlanStart.attributes['aria-invalid'],'true');assert.strictEqual(h.elements.aiPasteMealPlanBtn.disabled,true);assert.strictEqual(h.openCalls.length,0);
 ...
 h=harness({clipboardMode:'reject'});h.setActive('aiMealPlanScreen');h.elements.aiMealPlanStart.value='2026-08-20';h.elements.aiMealPlanEnd.value='2026-08-20';h.elements.aiGoalKeto.checked=true;h.elements.aiGenerateMealPlanBtn.dispatchEvent({type:'click'});await h.flush();
 assert.ok(h.elements.aiGeneratedMealPlanInstructions.value.includes('Date range: 2026-08-20 through 2026-08-20.'));assert.strictEqual(h.elements.aiMealPlanCopyStatus.textContent,'Automatic copying was blocked. Select and copy the instructions below.');assert.ok(h.elements.aiMealPlanCopyStatus.classList.contains('error'));assert.strictEqual(h.elements.aiPasteMealPlanBtn.disabled,true);h.elements.aiPasteMealPlanBtn.dispatchEvent({type:'click'});await h.flush();assert.strictEqual(h.elements.aiMealPlanPackage.value,'');assert.deepStrictEqual(h.reads,[]);
 h.elements.aiSelectMealPlanInstructionsBtn.dispatchEvent({type:'click'});assert.strictEqual(h.elements.aiGeneratedMealPlanInstructions.selected,true);h.elements.aiConfirmMealPlanPackageBtn.dispatchEvent({type:'click'});assert.strictEqual(h.elements.aiPasteMealPlanBtn.disabled,false);h.elements.aiPasteMealPlanBtn.dispatchEvent({type:'click'});await h.flush();assert.strictEqual(h.elements.aiMealPlanPackage.value,'Cloudflare build log');
