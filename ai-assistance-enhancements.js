...
function installEstimatedSaturatedFat(){
 ...
 function saturatedFatForDate(date){
  return (state.entries||[]).filter(e=>e&&e.date===date).reduce((sum,e)=>sum+num(e.food&&e.food.saturatedFat)*num(e.servings||1),0);
 }
 function updateMenuTotals(){
  const box=document.getElementById('menuTotals');
  if(!box)return;
  let pill=document.getElementById('estimatedSaturatedFatTotal');
  if(!pill){
   pill=document.createElement('div');
   pill.className='pill';
   pill.id='estimatedSaturatedFatTotal';
   box.appendChild(pill);
  }
  const date=typeof selectedDate==='function'?selectedDate():((document.getElementById('date')||{}).value||'');
  pill.innerHTML=`<b>${roundValue(saturatedFatForDate(date))}</b><span>Estimated Saturated Fat g</span>`;
 }
 function updateDailyTotalsTable(){
  const box=document.getElementById('totals');
  const table=box&&box.querySelector('.groupTotalsTable');
  if(!table||table.querySelector('tr[data-estimated-saturated-fat]'))return;
  const date=(document.getElementById('dailyTotalsDate')||{}).value||'';
  const groups=typeof totalsByGroupForDate==='function'?totalsByGroupForDate(date):[];
  if(!groups.length)return;
  const row=document.createElement('tr');
  row.dataset.estimatedSaturatedFat='1';
  const values=groups.map(g=>num(g.saturatedFat));
  const total=values.reduce((sum,value)=>sum+value,0);
  row.innerHTML=`<th scope="row">Estimated Saturated Fat <span class="small">(g)</span></th>${values.map(value=>`<td>${roundValue(value)}</td>`).join('')}<td class="dailyTotalCell"><b>${roundValue(total)}</b></td>`;
  const fatRow=[...table.querySelectorAll('tbody tr')].find(tr=>/^Fat\s*\(g\)/i.test((tr.querySelector('th')?.textContent||'').trim()));
  if(fatRow)fatRow.insertAdjacentElement('afterend',row);else table.querySelector('tbody')?.appendChild(row);
 }
 function refresh(){updateMenuTotals();updateDailyTotalsTable();}
 const observer=new MutationObserver(()=>refresh());
 const totals=document.getElementById('totals'),menuTotals=document.getElementById('menuTotals');
 if(totals)observer.observe(totals,{childList:true,subtree:true});
 if(menuTotals)observer.observe(menuTotals,{childList:true,subtree:true});
 document.getElementById('dailyTotalsDate')?.addEventListener('change',()=>setTimeout(refresh,0));
 document.getElementById('date')?.addEventListener('change',()=>setTimeout(refresh,0));
 refresh();
}
...
function install(){installAnalyzeMenu();installEstimatedSaturatedFat();}
