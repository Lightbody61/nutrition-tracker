...
function recipeBase(recipe,linked=false){
 ...
 return {name,servings,ingredients:recipe.ingredients.map((x,i)=>normalizeIngredient(x,i,linked)),notes:String(recipe.notes||''),containsEstimates:recipe.containsEstimates===true};
}
...
function renderReview(){
 ...
 if(pending.operation==='addFood'){const f=pending.food;html=`<p><b>${esc(f.name)}</b>${f.brand?` — ${esc(f.brand)}`:''}</p>${foodNutritionTable(f)}${pending.duplicate?'<p class="aiWarning">Duplicate warning: a food with this name and brand already exists. Saving is blocked.</p>':''}`;}
  ...
  const r=pending.recipe;html=`<p><b>${esc(r.name)}</b> · ${r.servings} servings</p>${pending.duplicate?'<p class="aiWarning">A recipe with this name already exists. Saving is blocked.</p>':''}<h3>Ingredient plan</h3>`;
  ...
  html+=`<h3>New foods available to create</h3>${proposedUsed.length?proposedUsed.map(f=>`<section class="aiIngredient"><b>${esc(f.name)}${f.brand?` — ${esc(f.brand)}`:''}</b>${foodNutritionTable(f)}</section>`).join(''):'<p>None. All ingredients reuse existing foods.</p>'}<div id="aiRecipeNutrition"></div>`;
 }else{const r=pending.recipe;html=`<p><b>${esc(r.name)}</b> · ${r.servings} servings</p>`+r.ingredients.map((item,i)=>{const matches=pending.matches[i],options=matches.map((f,j)=>`<option value="match:${j}">${esc(f.name)} (${esc(f.serving||'serving unknown')})</option>`).join('');return `<div class="aiIngredient"><b>${esc(item.amount)} ${esc(item.unit)} ${esc(item.name)}</b><select id="aiMatch${i}" data-ai-match="${i}"><option value="unresolved">Unresolved</option>${options}</select></div>`;}).join('')+`<div id="aiRecipeNutrition"></div>`;}
...
window.NutritionTrackerAI={parsePackage,validatePackage,validateFood,validateRecipe,validateRecipeWithFoods,findMatches,calculateRecipeNutrition,defaultResolutions,approveImport,undoImport,foodUsedElsewhere,schemaPrompt};
