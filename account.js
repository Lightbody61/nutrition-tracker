...
(function(){
  ...
  let loadInFlight=null,loadInFlightKey=null,recoveryContext=null,forumPosting=false,forumProfile=null,forumIsAdmin=false,forumComments=[],forumProfiles=new Map(),forumLikes=new Map(),forumAdminUsers=[],forumAdminComments=[],accountRequested=false;
  ...
  function setAuthView(user){
    ...
    if(!user){forumProfile=null;forumIsAdmin=false;window.forumAdminAccessConfirmed=false;forumComments=[];forumProfiles.clear();forumLikes.clear();renderCommunityForumComments([]);forumStatus('');if(el('forumAdministrationBtn'))el('forumAdministrationBtn').classList.add('hide');clearAdminState();stopPresenceHeartbeat();}
  }
  ...
  async function toggleForumLike(comment,button){const active=await requireForumSession();if(!active)return false;const users=forumLikes.get(comment.id)||new Set(),liked=users.has(active.user.id);button.disabled=true;try{const query=liked?client.from('forum_comment_likes').delete().eq('comment_id',comment.id).eq('user_id',active.user.id):client.from('forum_comment_likes').insert({comment_id:comment.id,user_id:active.user.id});const result=await query;if(result.error)throw result.error;if(liked)users.delete(active.user.id);else users.add(active.user.id);forumLikes.set(comment.id,users);renderCommunityForumComments();return true;}catch(error){forumStatus(forumErrorMessage(error),'error');button.disabled=false;return false;}}
  function renderForumComment(comment,isReply=false){const article=document.createElement('article'),meta=document.createElement('div'),text=document.createElement('div'),actions=document.createElement('div'),likes=forumLikes.get(comment.id)||new Set();article.id='forum-comment-'+comment.id;article.className='forumComment'+(isReply?' forumReply':'');meta.className='forumCommentMeta';text.className='forumCommentText';actions.className='forumCommentActions';let label=forumName(comment.user_id);if(isReply&&comment.reply_to_user_id)label+=' · Replying to '+forumName(comment.reply_to_user_id);meta.textContent=label+' · '+new Date(comment.created_at).toLocaleString();text.textContent=String(comment.comment_text||'');actions.appendChild(forumButton('Reply',()=>openReplyForm(comment),'secondary forumActionButton'));const likeLabel='Like'+(likes.size?' ('+likes.size+')':'');actions.appendChild(forumButton(likeLabel,event=>toggleForumLike(comment,event.currentTarget),'secondary forumActionButton'));article.append(meta,text,actions);return article;}
  ...
  async function loadCommunityForumComments(){const active=await requireForumSession('Log in to view Community Forum comments.');if(!active){forumComments=[];forumProfiles.clear();forumLikes.clear();renderCommunityForumComments();return false;}forumStatus('Loading Forum…');try{await loadForumProfile(active);await loadForumAdminStatus();const result=await client.from('community_forum_comments').select('id,user_id,parent_comment_id,reply_to_user_id,comment_text,created_at').order('created_at',{ascending:false}).limit(200);if(result.error)throw result.error;forumComments=result.data||[];const ids=[...new Set(forumComments.flatMap(comment=>[comment.user_id,comment.reply_to_user_id]).filter(Boolean))];forumProfiles=new Map();forumLikes=new Map();if(ids.length){const profiles=await client.from('forum_profiles').select('user_id,screen_name').in('user_id',ids);if(profiles.error)throw profiles.error;(profiles.data||[]).forEach(profile=>forumProfiles.set(profile.user_id,profile.screen_name));}if(forumComments.length){const likes=await client.from('forum_comment_likes').select('comment_id,user_id').in('comment_id',forumComments.map(comment=>comment.id));if(likes.error)throw likes.error;(likes.data||[]).forEach(like=>{if(!forumLikes.has(like.comment_id))forumLikes.set(like.comment_id,new Set());forumLikes.get(like.comment_id).add(like.user_id);});}renderCommunityForumComments();if(!forumProfile){beginForumScreenNameChange();forumStatus('Choose a screen name before posting or replying.','error');}else forumStatus('Comments refreshed.','success');return true;}catch(error){forumStatus(forumErrorMessage(error),'error');return false;}}
